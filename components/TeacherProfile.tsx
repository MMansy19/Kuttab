...
export default function TeacherProfile({ teacher }: TeacherProfileProps) {
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-8 flex flex-col gap-4 transition-colors duration-300 max-w-2xl mx-auto mt-8">
      <div className="flex items-center gap-6 mb-4">
        <img
          src={teacher.avatarUrl || '/avatar-placeholder.png'}
          alt={teacher.name}
          className="w-24 h-24 rounded-full border-2 border-emerald-300 dark:border-emerald-700 object-cover"
        />
        <div>
          <h2 className="text-2xl font-bold text-emerald-900 dark:text-white">{teacher.name}</h2>
      ...
      <div className="text-emerald-900 dark:text-emerald-200 text-right leading-loose">{teacher.bio}</div>
      <div className="flex flex-wrap gap-4 mt-2">
        <div className="px-4 py-2 rounded bg-emerald-100 dark:bg-emerald-900 text-emerald-900 dark:text-emerald-200">
      ...
      <div className="mt-4">
        ...
        <ul className="list-disc pr-6 mt-2 text-emerald-900 dark:text-emerald-200">
      ...
      {teacher.videoUrl ? (
        ...
        <div className="mt-6 text-emerald-700 dark:text-emerald-300 text-center">لا يوجد فيديو تعريفي</div>
      )}
    </div>
  );
}